...
void StudentTests() {
  ...
  assert (bdmap.size() == 6);
  bidirectional_map<std::string,int> bdmap_copy(bdmap);
  bdmap.print(std::cout);
  ...
  bidirectional_map<std::string,int> bdmap2;
  bdmap2 = bdmap;
  bdmap.print(std::cout);
  bdmap2.print(std::cout);

  bdmap2.insert(std::make_pair("ggg", 14));
  bdmap.print(std::cout);
  bdmap2.print(std::cout);
  assert(bdmap2 != bdmap);
  //   * bidirectional_maps with other data types
    std::cout << "\nDifferent data types" << std::endl;   
    std::vector<char> keys2;
  keys2.push_back('a');
  keys2.push_back('b');
  keys2.push_back('c');
  keys2.push_back('d');
  ...
  values2.push_back(2.4);
  values2.push_back(2.6);
  values2.push_back(2.8);

  bidirectional_map<char,double> bdmap3;
  assert (bdmap3.size() == 0);

  // INSERT 6 SIMPLE ONE-TO-ONE ASSOCIATIONS
  // (a later test case will verify the return value of insert)
  for (int i = 0; i < 4; i++) {
    bdmap3.insert(std::make_pair(keys2[i],values2[i])); 
  }
  bdmap3.print(std::cout);

  //   * all corner cases of insert, find, and erase
    bidirectional_map<std::string,int> bdmap4;
  assert (bdmap4.size() == 0);
  bdmap4.insert(std::make_pair("today", 1));
  assert(bdmap4.find("today")!=bdmap4.key_end());
  //bdmap4.erase("today"); 
  //assert (bdmap4.size() == 0);
