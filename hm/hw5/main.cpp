...
bool SanityCheckPrimary(Node* primary_order_top) {
  ...
  if (primary_order_top->before != NULL) {std::cout<<'1'<<std::endl;return false;}
  Node *tmp = primary_order_top;
  // walk through the list, checking forward & backward pointers
  while (tmp != NULL) {
    if (tmp->before != NULL) { if (tmp->before->after != tmp) {std::cout<<'2'<<std::endl;return false;} }
    if (tmp->after != NULL)  { if (tmp->after->before != tmp) {std::cout<<'3'<<std::endl;return false;} }
    tmp = tmp->after;
  }
  return true;
}
...
void SortingTest() {
  ...
  SanityCheckPrimary(deck);
  SanityCheckSorted(sorted);
  SanityCheckBoth(deck);
  // cleanup
  DeleteAllCards(deck);
...
void StudentTests() {
  ...
  //Shuffle test more than 52 cards
  PerfectShuffleTest(53,true);
  PerfectShuffleTest(1,true);

  //ShuffleDealing test
  Node* deck = CreateDeck(110);
  Node *top, *bottom;
  CutDeck(deck,top,bottom,"perfect");
  deck = Shuffle(top,bottom,"perfect");  
  CutDeck(deck,top,bottom,"perfect");
  ...
  PrintDeckPrimary("Aftershuffle", deck);
  Node* hands[5];
  Deal(deck,hands,5,"one-at-a-time",20);
  PrintDeckPrimary("person1:",hands[0]);
  PrintDeckPrimary("person2:",hands[1]);
  PrintDeckPrimary("person3:",hands[2]);
  PrintDeckPrimary("person4:",hands[3]);
  PrintDeckPrimary("person5:",hands[4]);
  PrintDeckPrimary("deck:",deck);
  Node* sorted_hands[5];
  sorted_hands[0] = SortHand(hands[0]);
  sorted_hands[1] = SortHand(hands[1]);
  ...
  PrintDeckSorted(" person1(sorted):",sorted_hands[0]);
  PrintDeckSorted(" person2(sorted):",sorted_hands[1]);
  PrintDeckSorted(" person3(sorted):",sorted_hands[2]);
  PrintDeckSorted(" person4(sorted):",sorted_hands[3]);
  PrintDeckSorted(" person5(sorted):",sorted_hands[4]);
  DeleteAllCards(hands[0]);
  DeleteAllCards(hands[1]);
  DeleteAllCards(hands[2]);
  DeleteAllCards(hands[3]);
