...
float multidivide(int numerator, int d1, int d2, int d3, int d4) {
  float f = ((((numerator*1.0 / d1) / d2) / d3) / d4);
  return f;
}
...
bool vector_compare(const std::vector<int> v1, const std::vector<int> v2) {
  bool success = true;
  for(uint i=0; i<std::min(v1.size(), v2.size()); ++i) {
    if(v1[i] <= v2[i]) {
      success = false;
    }
  }
...
*/
void corned_beef_hash(int* output, int input) {
  *(output+((input+1962) & 3)) = input ^ 3735928559; 
  *(output+((input+1492) & 3)) = input ^ 3056505882; 
  *(output+((input+'G') & 3)) = input ^ 01353340336; 
  *(output+((input+1) & 3)) = input ^ 3405691582; 
  int bitCtr = 1;
  char bit = 0;
