...
int solution=0; 
int prev_distance=999;
int MAXx, MAXy;
bool flag;
vector<vector<bool> > blocked_grid;
vector<vector<bool> > blocked_tmp;
...
void searchpath(int x, int y, int distance){
  if ( x < 0 || y < 0 || x > MAXx || y > MAXy) return;
  if ( blocked_grid[x][y] == true ) return;
  distance++;    
  if ( x == 0 && y == 0)  {
    if ( distance == prev_distance ) {solution++; return;}
  ...
  if ( distance<=prev_distance ){
    searchpath(x, y-1, distance);
    searchpath(x-1, y, distance);
    searchpath(x, y+1, distance);
    searchpath(x+1, y, distance);
  }
  blocked_grid[x][y]=false;
}

void storepath(int x, int y, int distance){
  if ( flag ) return;
  if ( x < 0 || y < 0 || x > MAXx || y > MAXy) return;
  if ( blocked_grid[x][y] == true ) return;
  distance--;  
  ...
  blocked_grid[x][y]=true;
  if ( distance>=0 ){
    storepath(x, y-1, distance);
    storepath(x-1, y, distance);
    storepath(x, y+1, distance);
    storepath(x+1, y, distance);
  }
  blocked_grid[x][y]=false;
...
int main(int argc, char* argv[]) {
  ...
  PT.clear();
  searchpath(start_x, start_y, -1);
  cout << solution << endl;
  flag = false;
  storepath(start_x, start_y, prev_distance+1);
  vector<Point>::reverse_iterator rit=PT.rbegin();
  for (; rit!=PT.rend(); ++rit)
    cout<<"("<<rit->x<<","<<rit->y<<")->";
  cout<<"(0,0)\n";
  return 0;
