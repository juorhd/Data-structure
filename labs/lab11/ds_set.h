...
public:
    ...
    T accumulate_( T init ) {
        init += root_->value;
        accumulate_( init, root_->left );
        accumulate_( init, root_->right );
        return init;
    }
    
    void accumulate_( T& result, TreeNode<T>* cur ) {
        if ( cur == NULL ) return;
        result += cur->value;
        accumulate_( result, cur->left );
        accumulate_( result, cur->right );
    }
